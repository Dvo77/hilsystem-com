 ...
 *           Guild Media Viewer (popup modal for guild_media assets — illustrations,
 *           diagrams, PATCH & SCRATCH shorts, printable PDFs — never inline in chat)
 *
...
const NAV_TOOLS = [
    ...
    media: {
      ...
      async open(assetId) {
        ...
        try {
          ...
          const docSnap = await db.collection('guild_media').doc(assetId).get();
          if (!docSnap.exists) throw new Error('Asset not found: ' + assetId);
  ...
  // Event delegation so .hil-media-card works even when injected dynamically
  // (e.g. into the PATCH chat drawer after a message renders).
  document.addEventListener('click', (e) => {
    const card = e.target.closest && e.target.closest('.hil-media-card');
