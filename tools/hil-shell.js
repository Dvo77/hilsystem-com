 ...
 *          items and museum pieces can reference via a person_id instead of
 *          duplicating provenance text per item. Top-level people/{personId}
 *          collection, same write class as museum_items — no admin-action
 *          endpoint needed. Shell itself is otherwise untouched by this change.
 *   v2.19 — BUGFIX (platform-wide, not new-feature): every NAV_TOOLS href was
 *          same-directory-relative ('./hil-vault-cloud.html'), which only ever
 *          resolved correctly when the CURRENT page also lived in tools/ root.
 *          Any page one level down — tools/foundations/*, tools/electric-forge/*,
 *          tools/natural-sciences/*, tools/weights-measures/* — rendered the
 *          exact same nav bar (buildNav() in this file is shared, no per-page
 *          override), so clicking any nav link from inside a hub subfolder
 *          resolved against THAT subfolder instead of tools/ root. Caught when
 *          the Tags nav link, clicked from a Foundations hub page, resolved to
 *          tools/foundations/hil-tag-manager.html instead of tools/hil-tag-
 *          manager.html. Fix: every href is now root-relative (leading /tools/),
 *          so it resolves identically regardless of which folder the current
 *          page is in. This was live and silently broken before Tag Manager
 *          existed — any nav click FROM inside a hub subfolder TO a tools/-root
 *          tool was already resolving wrong; nobody had happened to test that
 *          click path yet.
 *   v2.18 — Added Tag Manager to nav (id: 'tag-manager', ./hil-tag-manager.html),
 *          same "promoted to top-level tool" pattern as Vessel Builder (v2.10) and
 *          Kit Builder (v2.11). Manages the tag_registry taxonomy tree (parent/child
 *          category grouping, merge, type) that Vault's subcategory autocomplete reads
 *          from — cross-module infrastructure, not a Vault-only feature, hence its own
 *          nav slot rather than living inside the Vault modal.
 *   v2.17 — Header avatar is now a real composite (frame + background + photo),
 *          reading users/{uid}/avatar_profile/main and, if a frame is set, the
 *          matching avatar_catalog doc's "simple" weight art (header is 28px —
 *          always simple, never detailed). Falls back silently to the plain
 *          Google-photo circle if avatar_profile doesn't exist yet, so this is
 *          safe for every existing user who hasn't touched the Admin Panel
 *          avatar picker. BUGFIX: initializeUserRecord previously overwrote
 *          avatar_url with user.photoURL on every single login, silently
 *          wiping out any custom photo set via the Admin Panel — now guarded
 *          the same way birthday/preferences already were, only seeded once
 *          on first-ever record creation.
 *   v2.16 — Seasonal Flare renderer now supports image particles, not just
 *          emoji glyphs (set `image: url` instead of `glyphs` on a
 *          SEASONAL_EFFECTS entry). Added drift:'mixed' — each particle
 *          independently picks up or down at spawn instead of one uniform
 *          direction. New April Fools effect (Apr 1) uses both: Scratch
 *          sticker art, rising and falling chaotically, slow per-particle
 *          spin. Image lives at hilsystem.com/scratch-head-hero.png (repo
 *          root, confirmed live via GitHub, served by Cloudflare Pages —
 *          not R2/assets.hilsystem.com).
 *          Built and verified against the actual live v2.15 file (pulled
 *          fresh from GitHub), not an older local copy — Guild Media Library/
 *          renderForConcept work from v2.13–v2.15 is untouched.
 *   v2.15 — This build starts from the confirmed-live v2.13 (verified against
 *          Dan's actual deployed file, pasted back for comparison) and layers
 *          on exactly two changes, nothing else touched:
 *          (1) Guild Media Library panel — header "🖼 MEDIA" button (every
 *          page, shell-level, no per-page wiring) opens
 *          HILShell.media.openLibrary() — a standing grid of every active
 *          guild_media asset, independent of any page's concept tags. Click a
 *          thumbnail to open the existing single-asset viewer on top of it.
 *          Added as a more robust entry point after renderForConcept() proved
 *          hard to debug live — surfaces its own errors visibly in the panel
 *          body instead of failing silently (renderForConcept() still stays
 *          quiet on zero matches by design, but now logs to console).
 *          (2) Fixed R2_MEDIA_BASE — was pointed at assets.hilsystem.com,
 *          which per hilsystem-r2-signer.js's own header comment is the
 *          eventual custom domain but has unconfirmed DNS/CDN wiring, causing
 *          thumbnails/viewer images to silently fail to load. Now points at
 *          the raw pub-xxxx.r2.dev URL, same one hil-admin.html's own preview
 *          already uses successfully. One-line swap back once that domain's
 *          confirmed live.
...
(function () {
  ...
  const SEASONAL_EFFECTS = {
    ...
    // April Fools — Scratch sticker art instead of an emoji glyph. drift:'mixed'
    // means each particle independently picks up or down at spawn (chaotic,
    // not a uniform rise or fall). Swap `image` for the real R2 URL once the
    // sticker is uploaded to assets.hilsystem.com.
    fools:     { image: 'https://hilsystem.com/scratch-head-hero.png', count: 20, drift: 'mixed', sizeMin: 34, sizeMax: 54, speedMin: 30, speedMax: 65, sway: 35 },
  };
  ...
  function resolveSeasonalEffect(birthdayMMDD) {
    ...
    if (month === 4 && day === 1)  return 'fools';        // April Fools
...
const NAV_TOOLS = [
  ...
  const SHELL_CSS = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #hil-user-avatar-wrap.hil-avatar-composite { position: relative; width: 28px; height: 28px; flex-shrink: 0; }
    .hil-avatar-bg { position: absolute; inset: 14%; border-radius: 50%; }
    .hil-avatar-content { position: absolute; inset: 14%; border-radius: 50%; overflow: hidden; display: flex; align-items: center; justify-content: center; background: var(--hil-surface, #141816); }
    .hil-avatar-content img#hil-user-avatar { width: 100%; height: 100%; object-fit: cover; border-radius: 0; border: none; }
    .hil-avatar-frame { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
    ...
    */
    ...
    /* ═══════════════════════════════════════════════════════════════
       REPORT MODE — shared by any tool that generates a printable
       report (Insurance Report, Guild Progress, future Family List,
       etc). Wrap report content in these classes — or call
       HILShell.reportHeader()/reportFooter() below — and correct
       print behavior is guaranteed everywhere, forever. No individual
       tool has to know the shell's internal element ids to hide them.
       ═══════════════════════════════════════════════════════════════ */
    .hil-report-header { margin-bottom: 24px; border-bottom: 2px solid var(--hil-green); padding-bottom: 16px; }
    .hil-report-letterhead { font-family: var(--font-mono); font-size: 10px; letter-spacing: 0.2em; color: var(--hil-text-muted); text-transform: uppercase; margin-bottom: 6px; }
    .hil-report-title { font-family: var(--font-display); font-size: 22px; color: var(--hil-text); }
    .hil-report-subtitle { font-family: var(--font-ui); font-size: 14px; color: var(--hil-text-muted); margin-top: 2px; }
    .hil-report-meta { font-family: var(--font-mono); font-size: 11px; color: var(--hil-text-muted); margin-top: 10px; letter-spacing: 0.04em; }
    .hil-report-footer { margin-top: 24px; padding-top: 8px; border-top: 1px solid var(--hil-border); font-family: var(--font-mono); font-size: 9px; color: var(--hil-text-muted); letter-spacing: 0.04em; }

    @media print {
      /* Hides the app chrome on EVERY tool automatically, fixed once here —
         a prior per-tool attempt at this guessed wrong class names and hid
         nothing, which is the bug this centralizes the fix for. Any element
         with .no-print is hidden too, for tool-specific controls. */
      #hil-header, #hil-nav, #hil-toast-container, #patch-bubble, .no-print { display: none !important; }
      body { background: #fff !important; padding-top: 0 !important; color: #000 !important; }
      .hil-report-letterhead, .hil-report-meta, .hil-report-footer { color: #666 !important; }
      .hil-report-title, .hil-report-subtitle { color: #000 !important; }
      .hil-report-header { border-bottom-color: #000; }
      .hil-report-footer { border-top-color: #ccc; }
    }
  `;
  ...
  function buildHeader() {
    ...
    header.innerHTML = `
      ...
      <div id="hil-user-info">
        <div id="hil-user-avatar-wrap" class="hil-avatar-composite" style="display:none;">
          <div class="hil-avatar-bg" id="hil-avatar-bg"></div>
          <div class="hil-avatar-content" id="hil-avatar-content"><img id="hil-user-avatar" alt="avatar"/></div>
          <img class="hil-avatar-frame" id="hil-avatar-frame" alt="" style="display:none;"/>
        </div>
  ...
  // R2 public base — using the raw pub-xxxx.r2.dev URL, which is confirmed
  // working (same one hil-admin.html's own preview uses). assets.hilsystem.com
  // is the eventual custom domain per hilsystem-r2-signer.js's own header
  // comment, but its DNS/CDN wiring is unconfirmed — switch this one line
  // once that's verified live, no other code needs to change.
  const R2_MEDIA_BASE = 'https://pub-1a3ad1ec5a8042dc9cadf659089fe52c.r2.dev/';
  ...
  function playSeasonalFlare(effectType) {
    ...
    // Resolves which way ONE particle moves. Most effects just use spec.drift
    // directly ('up' or 'down'); drift:'mixed' (e.g. April Fools) picks a
    // random direction per particle instead, so some rise and some fall.
    function resolveDir(spec) {
      return spec.drift === 'mixed' ? (Math.random() < 0.5 ? 'up' : 'down') : spec.drift;
    }
    ...
    function spawnParticle(spec, canvas) {
      ...
      return {
        glyph: spec.glyphs ? spec.glyphs[Math.floor(Math.random() * spec.glyphs.length)] : null,
        dir,
        size,
        speed,
        x: Math.random() * canvas.width,
        y: dir === 'up' ? canvas.height + size + Math.random() * canvas.height * 0.5
                         : -size - Math.random() * canvas.height * 0.5,
        swayPhase: Math.random() * Math.PI * 2,
        swaySpeed: 0.5 + Math.random() * 1.2,
        rotation: Math.random() * 360,
        spin: (Math.random() - 0.5) * 40, // deg/sec — only used for image particles
      };
    }
    ...
    // Image-based effects (e.g. April Fools) need the sprite loaded before the
    // first draw; text/emoji effects need no preload and start immediately.
    if (spec.image) {
      const img = new Image();
      img.crossOrigin = 'anonymous';
      img.onload  = () => startLoop(img);
      img.onerror = () => {
        console.warn('[Seasonal Flare] Image particle failed to load, skipping effect:', spec.image);
        canvas.remove();
        window.removeEventListener('resize', resize);
      };
      img.src = spec.image;
  ...
  async function initializeUserRecord(user, setDoc, doc, getDoc) {
    try {
      ...
      // Read first so we never clobber `birthday`, `stats`, `created_at`, or an
      // existing `preferences.seasonal_flair_enabled` opt-out with defaults on
      // every login.
      let existing = null;
      try {
        const snap = await getDoc(ref);
      ...
      // `created_at` — BUGFIX: previously set unconditionally on every login,
      // silently resetting the account's real creation timestamp every time
      // someone signed in. Only set on first-ever record creation.
      if (!existing) {
        record.created_at = new Date();
      }

      // `stats` — BUGFIX: previously reset to all-zero on every login, wiping
      // the denormalized counters (item_count, collection_count, etc.) even
      // though the real subcollections underneath were untouched. Only
      // initialize on first-ever record creation; never overwritten by later
      // logins. Stat updates should happen wherever items/collections are
      // actually added, not here.
      if (!existing) {
        record.stats = { item_count: 0, collection_count: 0, star_count: 0, generosity_score: 0 };
      }

      // `avatar_url` — BUGFIX: previously this was unconditionally set to
      // user.photoURL on every login, silently overwriting any custom photo
      // uploaded via the Admin Panel avatar picker. Now matches the same
      // guard pattern as birthday/preferences below: only seed from Google's
      // photo on first-ever record creation, never touch it again after.
      if (existing?.avatar_url === undefined) {
        record.avatar_url = user.photoURL || null;
      } else {
  ...
  // ─── AVATAR COMPOSITE (frame + background + photo, header-size render) ────────
  // Reads users/{uid}/avatar_profile/main (activeFrame, activeBackground) and,
  // if a frame is set, the matching avatar_catalog doc for its "simple" weight
  // art (header render is small — 28px — so simple, not detailed). Falls back
  // silently to the plain Google-photo circle already showing if any of this
  // is missing, since most users won't have configured avatar prefs yet.
  const AVATAR_PALETTE_DEFAULT = '#00cc66';
  async function renderUserAvatarComposite(user, userData, fsFns) {
    const { doc, getDoc } = fsFns;
    try {
      const profileSnap = await getDoc(doc(firebaseDb, 'users', user.uid, 'avatar_profile', 'main'));
      const profile = profileSnap.exists() ? profileSnap.data() : {};

      const bgEl      = document.getElementById('hil-avatar-bg');
      const contentImg = document.getElementById('hil-user-avatar');
      const frameImg   = document.getElementById('hil-avatar-frame');
      if (!bgEl || !contentImg || !frameImg) return;

      // Background
      bgEl.style.background = profile.activeBackground || AVATAR_PALETTE_DEFAULT;

      // Content — photo if set, otherwise keep whatever's already showing
      // (Google photo from the immediate fallback, or nothing).
      const photoUrl = userData?.avatar_url || user.photoURL || null;
      if (photoUrl) contentImg.src = photoUrl;

      // Frame — only fetch if the profile actually has one set.
      if (profile.activeFrame) {
        const frameSnap = await getDoc(doc(firebaseDb, 'avatar_catalog', profile.activeFrame));
        if (frameSnap.exists()) {
          const frameData = frameSnap.data();
          if (frameData.simple?.file_r2_path) {
            const t = frameData.simple.transform || {};
            frameImg.src = frameData.simple.file_r2_path;
            frameImg.style.display = 'block';
            frameImg.style.transform =
              `translate(${t.offsetX || 0}px, ${t.offsetY || 0}px) scale(${t.scale || 1}) rotate(${t.rotation || 0}deg)`;
          }
        }
      }
    } catch (e) {
      // Non-fatal — header keeps showing the plain photo fallback already set.
      console.warn('HIL Shell: avatar composite render failed (showing fallback)', e.message);
    }
  }
  ...
  async function onAuthSuccess(user, setDoc, doc, getDoc, collection, query, where, getDocs) {
    ...
    const avatarWrap = document.getElementById('hil-user-avatar-wrap');
    const avatarImg   = document.getElementById('hil-user-avatar');
    const signout = document.getElementById('hil-signout-btn');
    // Show something immediately (Google photo or fallback icon) so the header
    // isn't empty while the composite fetch (avatar_profile + avatar_catalog)
    // is in flight — renderUserAvatarComposite below upgrades this in place.
    if (avatarWrap) {
    ...
    // ── Composite avatar render (frame + bg + photo) — upgrades the immediate
    //    Google-photo fallback above once avatar_profile/avatar_catalog resolve.
    renderUserAvatarComposite(user, userData, { doc, getDoc, collection, query, where, getDocs });
  ...
  async function loadFirebaseAndBoot() {
    ...
    onAuthStateChanged(firebaseAuth, async (user) => {
      if (user) {
        ...
        if (frameImg)   frameImg.style.display   = 'none'; // avoid stale frame flashing on next sign-in
        patchHistory = [];
        patchMemory  = '';
      }
    });
  ...
  const HILShell = {
    ...
    // ─── REPORT LETTERHEAD ───────────────────────────────────────────────
    // Standard header/footer markup for any printable report tool. Pairs
    // with the .hil-report-* CSS + @media print rules above — using these
    // instead of hand-written markup means every report looks the same
    // and print-hides correctly without each tool re-solving that.
    reportHeader({ title = '', subtitle = '', meta = '' } = {}) {
      return `<div class="hil-report-header">
        <div class="hil-report-letterhead">HIL SYSTEM</div>
        <div class="hil-report-title">${title}</div>
        ${subtitle ? `<div class="hil-report-subtitle">${subtitle}</div>` : ''}
        ${meta ? `<div class="hil-report-meta">${meta}</div>` : ''}
      </div>`;
    },

    reportFooter(note = 'HIL System · Generated document') {
      return `<div class="hil-report-footer">${note}</div>`;
    },
