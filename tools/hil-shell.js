...
(function () {
  ...
  // birthday (exact match) > Valentine's window > winter > fall.
...
const NAV_TOOLS = [
  ...
  async function maybeRunSeasonalFlare(user, userData, getDoc, doc) {
    try {
      ...
      const effect = resolveSeasonalEffect(userData?.birthday || null);
      if (!effect) return;
  ...
  // ─── AUTH SUCCESS ─────────────────────────────────────────────────────────────
  async function onAuthSuccess(user, setDoc, doc, getDoc) {
    ...
    maybeRunSeasonalFlare(user, userData, getDoc, doc);
  ...
  async function loadFirebaseAndBoot() {
    ...
    onAuthStateChanged(firebaseAuth, async (user) => {
      if (user) {
        await onAuthSuccess(user, setDoc, doc, getDoc);
      } else {
